function outputdata=DMINoiseCovariance(rawdata)
...
for m=1:NoChannels
    outputdata.concatenatednoise(:,m)=reshape(squeeze(outputdata.data.raw(:,m,:)),[size(outputdata.data.raw,1)*size(outputdata.data.raw,find(contains(outputdata.data.labels,'extr1'))) 1]);
end
