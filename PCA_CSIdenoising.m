function denoisedCSI=PCA_CSIdenoising(fid_data,patch_size,Parameters)
% PCA based denoising of CSI data
disp('Starting PCA based denoising on fid data')
disp(strcat('Patch size:',num2str(patch_size)))

NP=size(fid_data,1);
extended_data=repmat(fid_data,[1 patch_size patch_size patch_size]);

denoisedCSI=fid_data;
...
for mm=1:prod(Parameters.CSIdims)
    ...
    denoisedCSI(:,Grid_row,Grid_col,Grid_slice)=Phasecorrection(denoisedsignal);
